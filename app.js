(function () {
  ...
  async function clearOrders() {
    ...
    if (supabase) {
      try {
        // Supabase 不允許沒有條件的 delete()，所以我們用一個總是為真的條件 (例如 id 不為 null)
        // 或者如果你的表有其他欄位，也可以用。這裡使用 not('id', 'is', null) 是一種常見做法
        const { error } = await supabase
          .from('orders')
          .delete()
          .not(ORDERS_DB_ID_COL, 'is', null);
          
        if (error) throw error;
        cloudCleared = true;
      } catch (err) {
        console.error("Clear all orders failed on cloud:", err);
        cloudError = err;
      }
    }
    ...
    try {
      ...
      console.error("Clear local orders failed:", err);
    }
    ...
    if (cloudCleared && localCleared) {
      showToast("所有訂單記錄已清空 (包含雲端與本地)");
    } else if (localCleared && !cloudCleared) {
      const errorMsg = cloudError ? cloudError.message : "未知錯誤";
      showToast(`本地記錄已清空，但雲端同步失敗: ${errorMsg}`);
    } else if (!localCleared && cloudCleared) {
      showToast("雲端記錄已清空，但本地清除失敗");
    } else {
      showToast("清空記錄失敗，請稍後再試");
    }
  }
  ...
  const orderEditDialog = document.getElementById("order-edit-dialog");
  const orderEditForm = document.getElementById("order-edit-form");
  const closeOrderEdit = document.getElementById("close-order-edit");
  ...
  // 處理照片上傳錄入
  manualPhotoUpload?.addEventListener("change", async (e) => {
    const file = e.target.files?.[0];
    if (!file) return;

    // 簡單的大小檢查 (例如限制 2MB)
    if (file.size > 2 * 1024 * 1024) {
      showToast("照片太大（超過 2MB），請壓縮後再上傳。");
      e.target.value = "";
      return;
    }
    ...
    try {
      ...
      showToast("已添加上傳的照片到錄入清單");
      e.target.value = ""; // 清空以便再次上傳同一個檔案
    } catch (err) {
      console.error("Manual photo upload failed:", err);
      showToast("照片讀取失敗");
    }
  });
  ...
  closeOrderEdit?.addEventListener("click", () => {
  ...
  orderEditForm?.addEventListener("submit", async (e) => {
    e.preventDefault();
    const fd = new FormData(orderEditForm);
    const id = fd.get("id");
    ...
    if (idx !== -1) {
      orders[idx] = { 
        ...
        customerContact: updatedData.customer_contact 
      };
      localStorage.setItem(ORDERS_STORAGE_KEY, JSON.stringify(orders));
      localSuccess = true;
    }
    ...
    if (cloudSuccess || localSuccess) {
      showToast(cloudSuccess ? "訂單已更新並同步至雲端" : "雲端同步失敗，僅更新本地記錄");
      orderEditDialog.close();
      await renderOrderList();
    } else {
      showToast("更新失敗，請檢查網路或資料庫權限");
    }
  });
    ...
    if (supabase) {
      try {
        ...
        if (data && data.length > 0) {
          ...
          if (firstRow.hasOwnProperty('id')) ORDERS_DB_ID_COL = 'id';
          else if (firstRow.hasOwnProperty('db_id')) ORDERS_DB_ID_COL = 'db_id';
          else if (firstRow.hasOwnProperty('order_id')) ORDERS_DB_ID_COL = 'order_id';
          else {
            // 如果找不到標準 ID 欄位，輸出警告以供偵錯
            console.warn("未偵測到標準 ID 欄位，目前可用欄位有：", Object.keys(firstRow));
          }
        }

        return data.map(o => ({
          ...o,
          id: o[ORDERS_DB_ID_COL] || o.id, // 確保映射到通用 id 供前端使用
          batchId: o.batch_id,
        ...
        // 如果雲端抓取失敗，回傳本地數據作為後備
        try {
          let localOrders = JSON.parse(localStorage.getItem(ORDERS_STORAGE_KEY) || "[]");
          // 補丁：確保舊的本地記錄也有 ID
          let changed = false;
          localOrders = localOrders.map((o, idx) => {
            if (!o.id) {
              changed = true;
              return { ...o, id: `legacy-${idx}-${Date.now()}` };
            }
            return o;
          });
          if (changed) localStorage.setItem(ORDERS_STORAGE_KEY, JSON.stringify(localOrders));
          return localOrders;
        } catch (e) {
          return [];
        }
  ...
  async function saveOrdersBatch(customerName, items, customerContact = null) {
    ...
    const itemsToSave = items.map((item, idx) => ({
      id: item.id || `${batchId}-${idx}-${Math.random().toString(36).substr(2, 5)}`,
    ...
    if (supabase) {
      try {
        // Supabase 不一定需要我們提供 ID，如果資料表設定了自動生成，它會忽略
        // 但為了本地與雲端一致，我們可以嘗試傳入 ID
        const { error } = await supabase.from('orders').insert(itemsToSave.map(item => {
          const { id, ...rest } = item; // 如果 Supabase ID 是自增的，我們不傳入自定義 ID
          return rest;
        }));
        if (error) throw error;
        ...
        localStorage.setItem(ORDERS_STORAGE_KEY, JSON.stringify(orders));
        showToast("雲端同步失敗，已儲存於本地。請檢查資料庫權限。");
      }
    } else {
      const orders = JSON.parse(localStorage.getItem(ORDERS_STORAGE_KEY) || "[]");
      itemsToSave.forEach(item => orders.unshift({ ...item, time: timeStr, customerName: item.customer_name }));
  ...
  async function updateOrderStatus(id, newStatus) {
    ...
    if (supabase) {
      try {
        const { error } = await supabase.from('orders').update({ status: newStatus }).eq(ORDERS_DB_ID_COL, id);
        if (error) throw error;
        cloudSuccess = true;
      } catch (err) {
    ...
    if (cloudSuccess || localSuccess) {
      await renderOrderList();
      let msg = `訂單狀態已更新為：${newStatus}`;
      if (supabase && !cloudSuccess) msg += " (僅本地更新，雲端失敗)";
      showToast(msg);
    } else {
      const errorMsg = cloudError ? `更新失敗: ${cloudError.message || "資料庫錯誤"}` : "更新失敗：找不到該訂單記錄。";
      showToast(errorMsg);
    }
  }
  ...
  async function deleteOrder(id) {
    ...
    if (supabase) {
      try {
        const { error } = await supabase.from('orders').delete().eq(ORDERS_DB_ID_COL, id);
        if (error) throw error;
        cloudDeleted = true;
      } catch (err) {
    ...
    const newOrders = orders.filter(o => String(o.id) !== String(id));
    
    if (newOrders.length < initialLen) {
    ...
    if (cloudDeleted || localDeleted) {
      await renderOrderList();
      let msg = "記錄已刪除";
      if (supabase && !cloudDeleted) msg += " (僅本地刪除，雲端失敗)";
      showToast(msg);
    } else {
      const errorMsg = cloudError ? `刪除失敗: ${cloudError.message || "資料庫錯誤"}` : "刪除失敗：找不到該訂單記錄。";
      showToast(errorMsg);
    }
  }
        ...
        <tr class="order-row">
          ...
          <td data-label="操作">
            <div style="display: flex; gap: 8px; justify-content: center;">
              <button type="button" class="btn-order-edit" onclick="window.__XIAOJU_SURVEY__.editOrder(${safeId})" title="編輯記錄">
  ...
  adminLoginForm?.addEventListener("submit", (e) => {
    ...
    const rememberMe = fd.get("rememberMe");

    if (user === ADMIN_CREDENTIALS.user && pass === ADMIN_CREDENTIALS.pass) {
      isAdmin = true;
      if (rememberMe) {
        localStorage.setItem("xiaoju-admin-remembered", "true");
      } else {
        localStorage.removeItem("xiaoju-admin-remembered");
      }
  ...
  adminLogoutTrigger?.addEventListener("click", () => {
    isAdmin = false;
    localStorage.removeItem("xiaoju-admin-remembered"); // 登出時徹底清除記住狀態
    updateAdminUI();
    showToast("已登出管理員模式");
  });
  ...
  productEditForm?.addEventListener("submit", async (e) => {
    ...
    if (supabase) {
      try {
        const itemsToUpsert = productsToSave.map(p => ({
          type: type,
          title: p.title,
          sku: p.sku || null,
          spec_detail: p.specDetail || null,
          img: p.img || null,
          price: p.price || null,
        ...
        if (editId && !editId.startsWith('p-') && !editId.startsWith('pre-') && !editId.startsWith('arc-')) {
          // 編輯現有雲端產品
          const { error } = await supabase
            .from('products_db')
            .update(itemsToUpsert[0])
            .eq('id', editId);
          if (error) throw error;
        } else {
          // 新增產品
        ...
        showToast("商品已同步至雲端，所有人均可看到更新。");
        await loadProductsFromDB();
      } catch (e) {
        console.error("Supabase product save failed:", e);
        showToast(`同步到雲端失敗: ${e.message}`);
      }
    } else {
      // 本地儲存後備
      ...
      showToast(editId ? "商品修改成功 (本地)" : `成功新增 ${productsToSave.length} 個商品 (本地)`);
    }
  ...
  document.addEventListener("click", (e) => {
    ...
    // 點擊訂單清單縮圖
    if (e.target.closest(".order-thumb")) {
      openLightbox(e.target.src);
      return;
    }
    // 點擊公告板圖片
    if (e.target.closest(".billboard-img img")) {
      openLightbox(e.target.src);
      return;
    }
    // 點擊手動錄入訂單彈窗中的圖片預覽
    if (e.target.closest(".order-manual-img-preview img")) {
      openLightbox(e.target.src);
      return;
    }
    // 點擊商品編輯彈窗中的圖片預覽
    if (e.target.closest(".product-img-preview img")) {
      openLightbox(e.target.src);
      return;
    }
    // 點擊 Lightbox 圖片關閉 (返回上一頁)
    if (e.target === lightboxImg) {
      closeLightbox();
      return;
    }
    // 點擊 Lightbox 背景關閉 (針對 dialog 使用 showModal 的情況)
    if (e.target === lightbox) {
      ...
      // 如果點擊的是 dialog 本身 (通常是背景部分)，則關閉
      // 注意：如果 dialog 撐滿全螢幕，這可能需要調整
      closeLightbox();
    }
  });
  ...
  function switchMode(mode) {
    ...
    if (mode === "preorder") {
      ...
      // 切換到訂單頁面時，清空舊內容並顯示載入中（可選），確保抓取最新雲端資料
      if (ordersTbody) ordersTbody.innerHTML = `<tr><td colspan="8" style="text-align:center; padding:40px;">正在從雲端同步最新訂單...</td></tr>`;
      renderOrderList();
    } else {
      renderGrid();
    }
  ...
  async function loadProductsFromDB() {
    ...
    try {
      ...
      if (data) {
        // 獲取所有隱藏的預設商品 ID
        const hiddenIds = new Set(data.filter(p => p.is_hidden).map(p => p.db_id));

        // 分類存儲（排除隱藏項）
        const dbSurvey = data.filter(p => p.type === 'survey' && !p.is_hidden).map(p => ({
          ...p,
          id: p.db_id || p.id,
          specDetail: p.spec_detail,
          categoryLabel: p.category_label,
          isDefault: false
        }));
        const dbPreorder = data.filter(p => p.type === 'preorder' && !p.is_hidden).map(p => ({
          ...p,
          id: p.db_id || p.id,
          specDetail: p.spec_detail,
          isDefault: false
        }));
        const dbArchive = data.filter(p => p.type === 'archive' && !p.is_hidden).map(p => ({
          ...p,
          id: p.db_id || p.id,
          specDetail: p.spec_detail,
          isDefault: false
        ...
        // 更新全域變數
        const defaultSurvey = DEFAULT_PHOTO_ENTRIES.map((p, i) => ({
          id: `p-${i + 1}`,
          file: p.file,
          sku: `XJH-${String(i + 1).padStart(3, "0")}`,
          title: titleFromFilename(p.file),
        ...
        products = [...defaultSurvey, ...dbSurvey].sort((a, b) => (b.sortOrder || 0) - (a.sortOrder || 0));

        const defaultPre = DEFAULT_PREORDER_ENTRIES.map(p => ({ ...p, isDefault: true, sortOrder: 0 }))
          .filter(p => !hiddenIds.has(p.id));
        PREORDER_ENTRIES = [...defaultPre, ...dbPreorder].sort((a, b) => (b.sortOrder || 0) - (a.sortOrder || 0));

        const defaultArc = DEFAULT_ARCHIVE_FILES.map((file, i) => ({
          id: `arc-${String(i + 1).padStart(3, "0")}`,
          file,
          title: `歷史回顧 ${i + 1} (3/4/2026)`,
          img: `${ARCHIVE_DIR}/${file}`,
        ...
        ARCHIVE_ENTRIES = [...defaultArc, ...dbArchive].sort((a, b) => (b.sortOrder || 0) - (a.sortOrder || 0));
  ...
  async function init() {
    // 檢查是否有記住的登入狀態
    if (localStorage.getItem("xiaoju-admin-remembered") === "true") {
      isAdmin = true;
    }
  ...
  window.__XIAOJU_SURVEY__ = {
    ...
    editOrder: async (id) => {
      ...
      document.getElementById("edit-order-customer").value = order.customerName || "";
      document.getElementById("edit-order-contact").value = order.customerContact || "";
      document.getElementById("edit-order-title").value = order.title || "";
      document.getElementById("edit-order-sku").value = order.sku || "";
      document.getElementById("edit-order-qty").value = order.qty || 1;
      document.getElementById("edit-order-note").value = order.note || "";
      document.getElementById("edit-order-img").value = order.img || "";
    ...
    simulateOrder: (id, type = "survey") => {
      simulateOrder(id, type);
    },
    ...
    delProduct: async (id, type = "survey") => {
      ...
      if (supabase) {
        try {
          if (isDefault) {
            // 刪除預設商品：在雲端標記為 hidden
            const { error } = await supabase.from('products_db').insert({
              db_id: id, // 記錄原始 ID
              type: type,
              title: "HIDDEN_DEFAULT",
              is_hidden: true
            });
            if (error) throw error;
            showToast("預設商品已隱藏 (已同步)");
          } else {
            const { error } = await supabase.from('products_db').delete().eq('id', id);
            if (error) throw error;
            showToast("商品已從雲端刪除");
          }
          await loadProductsFromDB();
    ...
    topProduct: async (id, type = "survey") => {
      let currentList;
      if (type === "survey") currentList = products;
      else if (type === "preorder") currentList = PREORDER_ENTRIES;
      else currentList = ARCHIVE_ENTRIES;
      ...
      if (supabase) {
        try {
          if (isDefault) {
            ...
            // 現有雲端商品置頂
            const { error } = await supabase.from('products_db').update({ sort_order: newOrder }).eq('id', id);
            if (error) throw error;
          }
          showToast("商品已置頂 (已同步)");
          await loadProductsFromDB();
        } catch (e) {
          console.error("Top product failed:", e);
          showToast("置頂失敗");
        }
      } else {
        let storageKey = ADMIN_STORAGE_KEY;
        if (type === "survey") storageKey = ADMIN_STORAGE_KEY;
        else if (type === "preorder") storageKey = ADMIN_PREORDER_KEY;
        else storageKey = ADMIN_ARCHIVE_KEY;
        ...
        if (isDefault) {
          ...
          if (idx !== -1) adminProds[idx].sortOrder = newOrder;
        }
        localStorage.setItem(storageKey, JSON.stringify(adminProds));
