(function () {
  ...
  const ARCHIVE_ENTRIES = ARCHIVE_FILES.map((file, i) => ({
  ...
  if (archiveTotalEl) {
    archiveTotalEl.textContent = String(ARCHIVE_ENTRIES.length);
  }
  ...
  function renderPreorderGrid() {
    ...
    preorderGridEl.innerHTML = list
      ...
      <article class="product-card product-card--preorder" data-preid="${escapeHtml(p.id)}">
        <div class="thumb">
          <img src="预定照片/${preorderPhotoSrc(p.file)}" alt="${escapeHtml(p.title)}" loading="lazy" width="400" height="400" />
        </div>
        <div class="info">
          <h3 class="title">${escapeHtml(p.title)}</h3>
  ...
  document.querySelectorAll(".mode-tab").forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      if (mode === "survey" || mode === "preorder" || mode === "archive") switchMode(mode);
    });
  });
