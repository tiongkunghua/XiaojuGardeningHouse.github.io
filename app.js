(function () {
  ...
  let ANNOUNCEMENT;
  try {
    ANNOUNCEMENT = JSON.parse(localStorage.getItem(BILLBOARD_STORAGE_KEY));
  } catch (e) {
    console.error("Billboard content corrupted", e);
  }
  ...
  let selectedIds = new Set(); // 儲存管理員模式下選取的商品 ID

  const batchActionBar = document.getElementById("batch-action-bar");
  const batchCountNum = document.getElementById("batch-count-num");
  const btnBatchOrder = document.getElementById("btn-batch-order");
  ...
  function toggleSelect(id) {
    ...
    // 更新對應卡片的 UI
    const card = document.querySelector(`.product-card[data-id="${id}"], .product-card[data-preid="${id}"], .product-card[data-arcid="${id}"]`);
    if (card) {
      card.classList.toggle("is-selected", selectedIds.has(id));
      const checkbox = card.querySelector(".admin-select-check");
  ...
  function clearSelection() {
    selectedIds.clear();
    document.querySelectorAll(".product-card.is-selected").forEach(el => el.classList.remove("is-selected"));
    document.querySelectorAll(".admin-select-check").forEach(el => el.checked = false);
    updateBatchUI();
  }

  btnBatchCancel?.addEventListener("click", clearSelection);

  async function batchSimulateOrders() {
    if (selectedIds.size === 0) return;
    
    // 獲取所有選取的商品對象
    const allProducts = [...products, ...PREORDER_ENTRIES, ...ARCHIVE_ENTRIES];
    const selectedItems = Array.from(selectedIds).map(id => allProducts.find(p => p.id === id)).filter(Boolean);

    // 批量加入手動錄入列表
    selectedItems.forEach(item => {
      addManualOrderItem({
        title: item.title,
        sku: item.sku || (item.id.startsWith('pre-') ? 'PRE-ORDER' : (item.id.startsWith('arc-') ? 'ARCHIVE' : '')),
        img: item.img,
        qty: 1
      });
    });
    ...
    showToast(`已將 ${selectedItems.length} 項商品加入錄入清單`);
    clearSelection();
  }

  btnBatchOrder?.addEventListener("click", batchSimulateOrders);
    ...
    gridEl.innerHTML = list
      .map(
        (p) => {
          ...
          return `
      <article class="product-card ${isSelected ? 'is-selected' : ''}" data-id="${p.id}" onclick="${isAdmin ? `window.__XIAOJU_SURVEY__.toggleSelect('${p.id}')` : ''}">
        ${isAdmin ? `<input type="checkbox" class="admin-select-check" ${isSelected ? 'checked' : ''} onclick="event.stopPropagation(); window.__XIAOJU_SURVEY__.toggleSelect('${p.id}')">` : ''}
        <div class="thumb">
    ...
    preorderGridEl.innerHTML = list
      .map(
        (p) => {
          ...
          return `
      <article class="product-card product-card--preorder ${selectedIds.has(p.id) ? 'is-selected' : ''}" data-preid="${escapeHtml(p.id)}" onclick="${isAdmin ? `window.__XIAOJU_SURVEY__.toggleSelect('${p.id}')` : ''}">
        ${isAdmin ? `<input type="checkbox" class="admin-select-check" ${selectedIds.has(p.id) ? 'checked' : ''} onclick="event.stopPropagation(); window.__XIAOJU_SURVEY__.toggleSelect('${p.id}')">` : ''}
        <div class="thumb">
          <img src="${imgSrc}" alt="${escapeHtml(p.title)}" loading="lazy" width="400" height="400" />
        </div>
        <div class="info">
    ...
    archiveGridEl.innerHTML = list
      .map(
        (p) => {
          ...
          const isSelected = selectedIds.has(p.id);
          return `
      <article class="product-card product-card--archive ${isWanted ? 'is-wanted' : ''} ${isSelected ? 'is-selected' : ''}" data-arcid="${escapeHtml(p.id)}" onclick="${isAdmin ? `window.__XIAOJU_SURVEY__.toggleSelect('${p.id}')` : ''}">
        ${isAdmin ? `<input type="checkbox" class="admin-select-check" ${isSelected ? 'checked' : ''} onclick="event.stopPropagation(); window.__XIAOJU_SURVEY__.toggleSelect('${p.id}')">` : ''}
        <div class="thumb">
          <img src="${imgSrc}" alt="${escapeHtml(p.title)}" loading="lazy" width="400" height="400" />
        </div>
